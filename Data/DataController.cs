...
namespace ScraperService.Data
{
      ...
      public interface IDataController
      {
            ...
            List<TumBlog> GetBlogList(int daysFromLastScan);
            List<TumBlog> GetBlogList(int daysFromLastScan, int Qty);
            void UpdateImageLink(ImgLink img);
            void UpdateScanStatus(BlogScanStatus status);
            void UpdateScanStatus(TumBlog itm);
    ...
    "https://dn1.newtumbl.com/img/16580/13533097/1/19533221/nT_48ncngsxjby2vyiq2f112403.mp4"
            ...
            /*
                    ...
                    internal void InsertNewBlog(TumBlog tumBlog)
                    {
                        try
                        {
                            using LinkDBContext db = new();
                            SqlParameter blogid = new("@BlogID", tumBlog.BlogId);
                            SqlParameter blogname = new("@BlogName", tumBlog.BlogName);
                            SqlParameter blogurl = new("@BlogUrl", tumBlog.BlogUrl);

                            string cmd = $"exec InsertNewTumBlog(@blogID={tumBlog.BlogId},@BlogName={tumBlog.BlogName},@BlogUrl={tumBlog.BlogUrl})";
                            db.Database.ExecuteSqlRaw("exec InsertNewTumblog @blogId, @BlogName, @BlogUrl", blogid, blogname, blogurl);
                        }
                        catch (Exception ex)
                        {
                    ...
                    internal  Task<TumBlog> GetBlogIdByFileName(string filename)
                    {
                        TumBlog p = new();
                        using LinkDBContext db = new();
                        ...
                        if (query != null)
                        {
                            p = (from q in db.TumBlogs.Where(c => c.BlogId == query.BlogId) select q).SingleOrDefault();
                        }
            ...
            */
            ...
            /// <returns>List<TumBlog></returns>
            public List<TumBlog> GetBlogList(int daysFromLastScan)
            {
                  using LinkDBContext? db = new(_logger);

                  List<TumBlog> blog = (from x in db.TumBlogs
            ...
            public List<TumBlog> GetBlogList(int daysFromLastScan, int Qty)
            {
                  List<TumBlog> bloglist = new();
                  using LinkDBContext? db = new(_logger);
                  try
                  {
                        bloglist = (from x in db.TumBlogs
                                where x.LastScanDate <= DateTime.Now.AddDays(daysFromLastScan-(daysFromLastScan*2))
                                              && (x.IsEnabled == true || x.LastScanDate == null) select x).Take(Qty).ToList();


                    //    blog = (from x in db.TumBlogs where x.IsEnabled == true && (x.LastScanDate == null) select x).Take(Qty).ToList();
            ...
            public void UpdateScanStatus(TumBlog itm)
            {
                  try
                  {
                        using LinkDBContext db = new(_logger);
            ...
            {
                  using LinkDBContext db = new(_logger);
                  int cnt = (from x in db.TumBlogs where x.IsEnabled == true select x).Count();
                  return cnt;
